	...
	struct layer_par *par )
{
	...
	if(likely(lcdc_dev->clk_on))
	{
		...
		{
			case ARGB888:
				DBG(1, "enable per pixel alpha\n");
				LcdMskReg(lcdc_dev, WIN0_COLOR_KEY_CTRL, m_COLORKEY_EN | m_KEYCOLOR, v_COLORKEY_EN(0));
				LcdMskReg(lcdc_dev, DSP_CTRL0, m_W0_ALPHA_MODE, v_W0_ALPHA_MODE(1));
				LcdMskReg(lcdc_dev, BLEND_CTRL, m_W0_BLEND_EN, v_W0_BLEND_EN(1));
				break;
			default:
				DBG(1, "enable user defined alpha\n");
				LcdMskReg(lcdc_dev, DSP_CTRL0, m_W0_ALPHA_MODE, v_W0_ALPHA_MODE(0));
				LcdMskReg(lcdc_dev, BLEND_CTRL, m_W0_BLEND_EN, v_W0_BLEND_EN(0));		
				break;
		}
#endif
	...
	struct layer_par *par )
{
	...
	ScaleYrgbY = CalScale(yact, par->ysize);
	DBG(1,"%s for lcdc%d>>format:%d>>>xact:%d>>yact:%d>>xsize:%d>>ysize:%d>>xvir:%d>>yvir:%d>>xpos:%d>>ypos:%d>>scaleX:%d>>scaleY:%d\n",
		__func__,lcdc_dev->id,par->format,xact,yact,par->xsize,par->ysize,xvir,yvir,xpos,ypos,ScaleYrgbX,ScaleYrgbY);
