...
static int rk_request_fb_buffer(struct fb_info *fbi,int fb_id)
{
	...
	switch(fb_id)
	{
			...
			case 1:
				...
				fbi->screen_base = ioremap(res->start, fbi->fix.smem_len);
				memset(fbi->screen_base, 0, fbi->fix.smem_len);
				printk("fb%d:phy:%lx>>vir:%p>>len:0x%x\n",fb_id, fbi->fix.smem_start,fbi->screen_base,fbi->fix.smem_len);
				break;			
			case 3:
				fbi->fix.smem_start = fb_inf->fb[1]->fix.smem_start;
				fbi->fix.smem_len   = fb_inf->fb[1]->fix.smem_len;
				fbi->screen_base    = fb_inf->fb[1]->screen_base;
				printk("fb%d:phy:%lx>>vir:%p>>len:0x%x\n",fb_id, fbi->fix.smem_start,fbi->screen_base,fbi->fix.smem_len);
				break;			
#endif			
        	default:
