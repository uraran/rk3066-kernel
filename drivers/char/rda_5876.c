 ...
 * File:        drivers/char/rda_bt_dev.c
 */
...
#if 0
#define D_PRINTF(...)			printk("rda5876: " __VA_ARGS__)
#else
#define D_PRINTF(...)
#endif
...
static struct rda_bt_platform_data *rda_bt_pdata = NULL;
...
static void rda_bt_work_fun(struct work_struct *work)
{
    ...
    if(hdev == NULL)
    {
        ...
        //D_PRINTF(KERN_ALERT "hdev is NULL\n ");
    }
    else
    {
        D_PRINTF(KERN_ALERT "Send host wakeup command.\n");
...
static int rda_bt_dev_open(struct inode *inode, struct file *file)
{
    D_PRINTF("[## BT ##] rda_bt_dev_open.\n");
    eint_gen  = 0;
    eint_mask = 0;
    return 0;
}

static int rda_bt_dev_release(struct inode *inode, struct file *file)
{
    D_PRINTF("[## BT ##] rda_bt_dev_release.\n");
    eint_gen  = 0;
    eint_mask = 0;
    return 0;
}
...
static unsigned int rda_bt_dev_poll(struct file *file, poll_table *wait)
{
    uint32_t mask = 0;
    
    D_PRINTF("[## BT ##] rda_bt_poll eint_gen %d, eint_mask %d ++\n", eint_gen, eint_mask);
...
static int rda_bt_power_control(int on_off)
{    
    D_PRINTF("[## BT ##] rda_bt_power_control input[%d].\n", on_off);
	
    if(on_off)
    {	    
     	 gpio_direction_output(rda_bt_pdata->power_gpio.io, rda_bt_pdata->power_gpio.enable);
    	 msleep(500);
        enable_irq(irq_num); 
    }
    else
...
static long  rda_bt_dev_ioctl(struct file *file, unsigned int cmd,unsigned long arg)
{
    ...
    switch(cmd)
    {
		...
		{
				D_PRINTF("[## BT ##] IOCTL_BT_DEV_POWER cmd[%d] parm1[%d].\n", cmd, rate);
				if (copy_from_user(&rate, argp, sizeof(rate)))
				return -EFAULT;

				mutex_lock(&sem);  
				ret = rda_bt_power_control(rate);
				mutex_unlock(&sem);			
				break;
		}		
        ...
        case IOCTL_BT_SET_EINT:
        {
	        if (copy_from_user(&rate, argp, sizeof(rate)))
                return -EFAULT;
            D_PRINTF("[## BT ##] IOCTL_BT_SET_EINT cmd[%d].\n", cmd);
            mutex_lock(&sem); 
        ...
        default :
        {
	         D_PRINTF("[## BT ##] rda_bt_dev_ioctl cmd[%d].\n", cmd);
	         break;
	    }
    }
...
struct file_operations rda_bt_dev_ops = 
{
    .owner           = THIS_MODULE,
    .unlocked_ioctl  = rda_bt_dev_ioctl,
    .open            = rda_bt_dev_open,
    .release         = rda_bt_dev_release,
//    .poll            = rda_bt_dev_poll,
};
...
static int rda_bt_probe(struct platform_device *pdev)
{
    int ret;
    struct rda_bt_platform_data *pdata = pdev->dev.platform_data;
    
    D_PRINTF("rda_bt_probe\n");

    if(pdata == NULL) {
    	D_PRINTF("mt6622_probe failed.\n");
    	return -1;
    }
    rda_bt_pdata = pdata;
    ...
    if(ret < 0)
    {
        D_PRINTF("bt_host_wake irq request fail.\n");
    	  irq_num = -1;
        goto error;
    }
    ...
    ret = register_chrdev(BT_DEV_MAJOR_NUM, DEV_NAME, &rda_bt_dev_ops);
    if(ret < 0)
    {
        D_PRINTF("[## BT ##] [%d]fail to register the character device.\n", ret);
        goto error;
    }
    ...
    {
        D_PRINTF("BT RDA class_create failed\n");
        goto error;
    }
    ...
    if (!rda_bt_workqueue)
    {
        D_PRINTF("create_singlethread_workqueue failed.\n");
        ret = -ESRCH;
        goto error;
    }  
...
static int rda_bt_remove(struct platform_device *pdev)
{
    D_PRINTF("[## BT ##] cleanup_module.\n");
...
static struct platform_driver rda_bt_driver = {
        .probe = rda_bt_probe,
        .remove = rda_bt_remove,
        .driver = {
                .name = "rda5876",
                .owner = THIS_MODULE,
        },
...
static int __init rda_bt_init_module(void)
{
    D_PRINTF("Enter %s\n", __func__);
    return platform_driver_register(&rda_bt_driver);
}

static void __exit rda_bt_cleanup_module(void)
{
    D_PRINTF("Enter %s\n", __func__);
    platform_driver_unregister(&rda_bt_driver);
}

module_init(rda_bt_init_module);
module_exit(rda_bt_cleanup_module);
MODULE_DESCRIPTION("rda_bt_DEV");
