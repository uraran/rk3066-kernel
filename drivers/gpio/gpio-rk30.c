...
struct rk30_gpio_bank rk30_gpio_banks[] = {
	RK30_GPIO_BANK(0),
	RK30_GPIO_BANK(1),
	RK30_GPIO_BANK(2),
	RK30_GPIO_BANK(3),
...
{
//	printk("GPIOSetPinLevel: %X, %X, %d\n", (unsigned int)regbase, bit, level);
...
{
	int rv = ((__raw_readl(regbase + GPIO_EXT_PORT) & bit) != 0);
//	printk("GPIOGetPinLevel: %X, %X, %d\n", (unsigned int)regbase, bit, rv);
	return rv;
}

static void GPIOSetPinDirection(void __iomem *regbase, unsigned int bit, eGPIOPinDirection_t direction)
{
//	printk("GPIOSetPinDirection: %X, %X, %d\n", (unsigned int)regbase, bit, direction);
